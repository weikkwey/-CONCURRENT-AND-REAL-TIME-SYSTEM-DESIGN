...
sem_t semSmoker[5]; //semaphore for smokers
sem_t semVendor; //vendors semaphore
int smokeCount=-1;
int semWait[5]; //count for the number of cigarettes smoked done by each smokers 
...
string vendorsSets[]={"Tabacoo and Paper", "Paper and Matches", "Matches and Tabacoo", "Tabacoo and Paper", "Paper and Matches" };
...
void *cigarette(void *pVoid){
    ...
    for (int i = 1; i <=3; ++i){
        ...
        if ( smokeCount == 0 ){
            ...
            usleep(rand()%50000);
            cout<<"\033[0;35m==> \033[0;33mSmoker4 is creating a cigarette\033[0;0m"<<endl;
    		sleep(1);
	        cout<<"\033[0;31mSmoker4 is Smoking\033[0m \n";
	        semWait[3]--;
            sem_post(&semVendor);
            cout<<"\033[0;35m==> \033[0;33mSmoker4 done finish smoking\033[0;0m"<<endl;
        }
	else if ( smokeCount == 4 ){
            usleep(rand()%50000);
            cout<<"\033[0;35m==> \033[0;33mSmoker5 is creating a cigarette\033[0;0m"<<endl;
    		sleep(1);
	        cout<<"\033[0;31mSmoker5 is Smoking\033[0m \n";
	        semWait[4]--;
            sem_post(&semVendor);
            cout<<"\033[0;35m==> \033[0;33mSmoker5 done finish smoking\033[0;0m"<<endl;
        }
    }
    pthread_exit(NULL);
}
...
void *vendor(void *pVoid){
    ...
    for (int i = 0; i < 15; ++i){
	    ...
	    if( semWait[0]==0 && semWait[1]==0 && semWait[2]==0 && semWait[3]==0 && semWait[4]==0){
	        checkSmoked= true;
                break;
            }
...
int main(void){
    ...
    sem_init(&semSmoker[3],0,0);
    sem_init(&semSmoker[4],0,0);
    sem_init(&semVendor,0,1);
    sem_init(&vendorWake,0,0);
    sem_init(&smokerWake,0,0);
   
    pthread_t smokerA,smokerB,smokerC,smokerD,smokerE,vendorA,vendorB,pusher;
    ...
    pthread_create(&smokerD,NULL,cigarette,at3);
    pthread_create(&smokerE,NULL,cigarette,at4);
    pthread_create(&vendorA,NULL,vendor,at);
    pthread_create(&vendorB,NULL,vendor,at1);
    pthread_create(&pusher,NULL,func,NULL);
    
    pthread_join(smokerA,NULL);
    pthread_join(smokerB,NULL);
    pthread_join(smokerC,NULL);
    pthread_join(smokerD,NULL);
    pthread_join(smokerE,NULL);
